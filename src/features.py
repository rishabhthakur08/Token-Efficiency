...
def answer_comprehension_count(code: str) -> int:
    return len(re.findall(r'\[.+for.+in', code))

def answer_nested_loop_count(code: str) -> int:
    return len(re.findall(r'for.+\n.+for|while.+\n.+for', code))

def answer_conditional_count(code: str) -> int:
...
def extract_features(row: dict, dataset: str) -> dict:
    ...
    return {
        ...
        "answer_comprehension_count": answer_comprehension_count(code) if is_coding else 0,
        "answer_nested_loop_count":  answer_nested_loop_count(code) if is_coding else 0,
        "answer_conditional_count":  answer_conditional_count(code) if is_coding else 0,
    }